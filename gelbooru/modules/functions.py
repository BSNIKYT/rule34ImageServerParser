...
def parsing(text, Guard):
    ...
    for tag in soup.find_all('post'):
        tag= str(tag)#.replace('<post>', '').replace('</post>', '')
        # print(tag)
        
        # try:
        file_url = tag.split('file_url')[1].replace('>', '').replace('</', '')
        # print(file_url)
        tags = formating_tags_list(tag.split('tags')[1].replace('>', '').replace('</', ''), ind=0)
        # print(tags)
        _id = tag.split('id')[1].replace('>', '').replace('</', '')
        # print(_id)
        # raise
        ...
        if val:
            qq.append(
                {f'{_id}': {'id': _id, 'file_url': file_url, 'tags': tags}})
...
def find_count(_url, dev=False):
    ...
    # aaa = str(soup).split('''" limit="100" offset="''')[0].split('''<?xml version="1.0" encoding="utf-8"?>\n<posts count="''')[1]
    # print(aaa)
    # raise
    count = str(soup).split('''" limit="100" offset="''')[0].split('''<?xml version="1.0" encoding="utf-8"?>\n<posts count="''')[1]
    offset = 0
...
def reload_file_config(working_directory):
    ...
    if not os.path.exists(working_directory + dir_pref + statictics_file_name):
        ...
        q.append({
            ...
            'GuardTagsInverse' : GuardTagsInverse,
